...
export default function PharmacyIPReports() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setRecords(data.filter(r => r.saleType === 'IP'));
    } catch (e: any) {
      setError(e.message ?? 'Failed to load pharmacy IP reports');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Pharmacy IP Reports</h2>
          <p className="text-sm text-gray-500 mt-0.5">Inpatient medicine dispensing and billing records</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total IP Sales', value: records.length, color: 'bg-purple-50 text-purple-600' },
          { label: 'Paid', value: records.filter(r => r.status === 'Paid').length, color: 'bg-green-50 text-green-600' },
          { label: 'Pending / Credit', value: records.filter(r => r.status !== 'Paid').length, color: 'bg-yellow-50 text-yellow-600' },
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="relative">
          ...
          <input type="text" placeholder="Search IP sales…" value={search} onChange={e => setSearch(e.target.value)}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {['#', 'Sale ID', 'Patient', 'Admission ID', 'Medicine', 'Qty', 'Total (₹)', 'Date', 'Prescribed By', 'Status'].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.length === 0 ? (
                  <tr><td colSpan={10} className="px-4 py-12 text-center text-gray-400 text-sm">No IP pharmacy records found</td></tr>
                ) : filtered.map((r, idx) => (
                  <tr key={r.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-gray-600">{r.prescribedBy || '—'}</td>
