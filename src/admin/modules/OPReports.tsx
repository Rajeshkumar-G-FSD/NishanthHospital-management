...
import { RefreshCw, Search, Download, ClipboardList, Users, CheckCircle, Clock } from 'lucide-react';
...
export default function OPReports() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const data = await getAll('opdVisits');
      setRecords(data);
    } catch (e: any) {
      setError(e.message ?? 'Failed to load OP reports');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = records.filter(r => {
    ...
    const matchesFrom = !dateFrom || r.visitDate >= dateFrom;
    const matchesTo = !dateTo || r.visitDate <= dateTo;
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">OP Reports</h2>
          <p className="text-sm text-gray-500 mt-0.5">Outpatient visit records and consultation reports</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total OP Visits', value: stats.total, icon: <ClipboardList className="w-5 h-5" />, color: 'bg-blue-50 text-blue-600' },
          { label: 'Completed', value: stats.completed, icon: <CheckCircle className="w-5 h-5" />, color: 'bg-green-50 text-green-600' },
          { label: 'Scheduled', value: stats.scheduled, icon: <Clock className="w-5 h-5" />, color: 'bg-indigo-50 text-indigo-600' },
          { label: 'Cancelled', value: stats.cancelled, icon: <Users className="w-5 h-5" />, color: 'bg-red-50 text-red-600' },
        ].map(s => (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Visit ID</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Patient ID</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Doctor</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Visit Date</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Type</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Diagnosis</th>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-12 text-center text-gray-400 text-sm">
                      No OP visit records found
                    </td>
                  </tr>
                ) : filtered.map((r, idx) => (
                  <tr key={r.id} className="hover:bg-gray-50/50">
                    <td className="px-4 py-3 text-gray-300 text-xs">{idx + 1}</td>
                    <td className="px-4 py-3 text-gray-700 font-medium">{r.visitId || '—'}</td>
                    <td className="px-4 py-3 text-gray-600">{r.patientId || '—'}</td>
                    <td className="px-4 py-3 text-gray-700">{r.doctorId || '—'}</td>
                    <td className="px-4 py-3 text-gray-600">{r.visitDate || '—'}</td>
                    <td className="px-4 py-3 text-gray-600">{r.consultationType || '—'}</td>
                    <td className="px-4 py-3 text-gray-600 max-w-[180px] truncate" title={r.diagnosis}>{r.diagnosis || '—'}</td>
