...
import { RefreshCw, Search, Download, Scan, CheckCircle, Clock } from 'lucide-react';
...
export default function RadiologyReports() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const data = await getAll('radiology');
      setRecords(data);
    } catch (e: any) {
      setError(e.message ?? 'Failed to load radiology reports');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Radiology Reports</h2>
          <p className="text-sm text-gray-500 mt-0.5">X-Ray, Ultrasound, CT Scan, MRI and other imaging reports</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total Tests', value: records.length, color: 'bg-purple-50 text-purple-600' },
          { label: 'Completed', value: records.filter(r => r.status === 'Completed').length, color: 'bg-green-50 text-green-600' },
          { label: 'Pending', value: records.filter(r => r.status === 'Pending' || r.status === 'Ordered').length, color: 'bg-yellow-50 text-yellow-600' },
          { label: 'X-Ray / Ultrasound', value: records.filter(r => r.testType === 'X-Ray' || r.testType === 'Ultrasound').length, color: 'bg-blue-50 text-blue-600' },
        ].map(s => (
          <div key={s.label} className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-3 shadow-sm">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${s.color}`}><Scan className="w-5 h-5" /></div>
            <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="relative">
          ...
          <input type="text" placeholder="Search radiology…" value={search} onChange={e => setSearch(e.target.value)}
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          ...
          {['All', 'Ordered', 'In Progress', 'Completed', 'Cancelled'].map(s => (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {['#', 'Order ID', 'Patient ID', 'Test Name', 'Test Type', 'Doctor', 'Order Date', 'Result', 'Status'].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400 text-sm">No radiology records found</td></tr>
                ) : filtered.map((r, idx) => (
                  <tr key={r.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-gray-500 max-w-[140px] truncate" title={r.result}>{r.result || '—'}</td>
