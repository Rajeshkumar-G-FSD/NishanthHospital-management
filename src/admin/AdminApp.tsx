...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'laboratory',   label: 'Lab',          icon: <FlaskConical className="w-4 h-4" />,   collection: 'labTests' },
  { key: 'pharmacy',     label: 'Pharmacy',     icon: <Pill className="w-4 h-4" />,           collection: 'medicines' },
  { key: 'hr',           label: 'Staff',        icon: <UserCheck className="w-4 h-4" />,      collection: 'employees' },
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 shrink-0" style={{ height: 64, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        {mobile ? (
          ...
          <button
            onClick={onCollapse}
            className="absolute right-0 translate-x-full top-5 bg-white shadow-md rounded-r-md p-1 text-gray-500 hover:text-blue-600 transition-colors z-10"
          >
      ...
      <nav className="flex-1 overflow-y-auto" style={{ padding: '12px 8px' }}>
        {isExpanded && (
          <p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'rgba(160,174,192,0.5)' }}>
            Management Modules
          </p>
        )}
        {!isExpanded && <div className="my-2 mx-2" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }} />}
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              ...
              >
                ...
                {isExpanded && (
                  <>
                    <span className="truncate text-left font-medium flex-1">{item.label}</span>
                    {count !== null && (
                      <span
...
function AdminLayout({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    const collections = ['appointments', 'patients', 'doctors', 'schedules', 'invoices', 'inventory', 'labTests', 'medicines', 'employees', 'emergencyCases'];
  ...
  const activeLabel = NAV_ITEMS.find(i => i.key === activeModule)?.label ?? 'Dashboard';
